...
data_TMP[,TARGET:=shift(x4,predict_days,type="lead")]
...
if(simple){
  ...
  unlink(file.path(PATH_HOME,"output",paste0("204_",COIN_NAME),"*"))
  h2o.saveModel(ml,path=file.path(PATH_HOME,"output",paste0("204_",COIN_NAME)),force = T)
}else{
  ml = h2o.automl(
  ...
  unlink(file.path(PATH_HOME,"output",paste0("204_",COIN_NAME),"*"))
  h2o.saveModel(ml@leader,path=file.path(PATH_HOME,"output",paste0("204_",COIN_NAME)),force = T)
  ml = ml@leader
}
...
Y = data[,list(date=as.character(date),low=low)]
YHAT = data.table(date=as.character(new_date),low=pred$predict)
YHAT = rbind(tail(Y,1),YHAT)
data.table(tail(Y),head(YHAT))
saveRDS(list("Y"=Y,"YHAT"=YHAT),file.path(PATH_HOME,"output",paste0(COIN_NAME,"_low.Rda")))
